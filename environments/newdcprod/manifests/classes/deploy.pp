class deploy {
    ...
    exec { "infra-cli-command":
    	command => "reposervice --host repo-svc-app-0001.nm.flipkart.com --port 8080 getenv --name $envName --appkey 12 --version $envVersion > /etc/apt/sources.list.d/source1.list",
	    path => "/usr/bin/",
